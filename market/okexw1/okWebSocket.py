...
class okWSTool():
    ...
    def openFutureTicker(self,X = 'btc',Y = 'this_week'):#默认使用季合约#ok_sub_futureusd_X_ticker_Y
        chanelcmd = "{'event':'addChannel','channel':'ok_sub_futureusd_%s_ticker_%s'}"%(X,Y)
        self.wsocket.send(chanelcmd)
    #期货成交数据推送
    # ① X值为：btc, ltc, eth, etc, bch,eos,xrp,btg 
    # ② Y值为：this_week, next_week, quarter
    def openFutureData(self,X = 'btc',Y = 'this_week'):#默使用季合约成交数据
        chanelcmd = "{'event':'addChannel','channel':'ok_sub_futureusd_%s_trade_%s'}"%(X,Y)
        self.wsocket.send(chanelcmd)
    #期货200深度增量数据推送,
    # ① X值为：btc, ltc, eth, etc, bch,eos,xrp,btg 
    # ② Y值为：this_week, next_week, quarter
    def openFutureDepth200(self,X = 'btc',Y = 'this_week'):
    ...
    # ② Y值为：this_week, next_week, quarter 
    # ③ Z值为：5, 10, 20(获取深度条数)
    def openFutureDepth(self,X = 'btc',Y = 'this_week',Z = 5):
