...
import OKFutureWebSocket

#初始化apikey，secretkey,url
apikey = apikeytool.apikey
secretkey = apikeytool.secretkey
okcoinRESTURL = 'www.okex.com'#'www.okcoin.com'   #请求注意：国内账号需要 修改为 www.okcoin.cn  

#期货API
# okcoinFuture = OKCoinFuture(okcoinRESTURL,apikey,secretkey)

okwsClient = OKFutureWebSocket.OKFutureWSClient()
...
class TradeTool(object):
    ...
    def __init__(self,amount = 30,isTest = False):
        #期货交易工具
        global okwsClient
        self.okTradeTool = OKCoinFuture(okcoinRESTURL,apikey,secretkey)
        self.okWS = okwsClient
        self.depthSells = []  #卖方深度
        self.depthBuys = []   #买方深度
        self.amount = amount  #默认下单交易量,以100美元为单
        self.isTest = isTest  #是否模拟方式运行
        self.IDs = []         #下单ID
        self.isOpen = False   #是否已下单
